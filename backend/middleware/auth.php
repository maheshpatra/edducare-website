...
class AuthMiddleware
{
    ...
    public function authenticate()
    {
        ...
        // Support for token in query parameter or post body (useful for file downloads via window.open)
        if (!$token) {
            if (isset($_GET['token'])) {
                $token = $_GET['token'];
            } elseif (isset($_GET['auth_token'])) { // Also support auth_token for consistency with localStorage
        ...
        if (!$token) {
            return false; // Let the endpoint handle the error response
        }
        ...
        try {
            ...
            if (!$tokenData) {
                ...
                echo json_encode([
                    'error' => 'Invalid or expired token (not found in auth_tokens table or expired/revoked)',
            ...
            if ($userStmt->rowCount() === 0) {
                http_response_code(401);
                echo json_encode(['error' => 'User not found or inactive (ID: ' . $payload['user_id'] . ')']);
                return false;
            }
            ...
            if ($user['role_name'] === null) {
                http_response_code(401);
                echo json_encode(['error' => 'Role mismatch: user has no linked role_name']);
                return false;
            }
    ...
    private function getAllHeaders()
    {
        ...
        // Essential fallback for Apache CGI/FastCGI environments
        if (!isset($headers['Authorization'])) {
    ...
    public function requireStudentAccess($studentId = null)
    {
        ...
        if (in_array($user['role'], ['admin', 'school_admin', 'teacher_academic', 'teacher_administrative', 'teacher', 'accountant', 'librarian'])) {
            if ($studentId) {
    ...
    public function logActivity($user, $action, $entityType = null, $entityId = null, $oldValues = null, $newValues = null)
    {
        try {
            ...
            $logStmt->bindValue(':old_values', $oldValues ? json_encode($oldValues) : null);
            $logStmt->bindValue(':new_values', $newValues ? json_encode($newValues) : null);
