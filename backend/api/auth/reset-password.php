...
try {
    ...
    $tokenQuery = "SELECT prt.*, u.id as user_id, u.email 
                   FROM password_reset_tokens prt
                   JOIN users u ON prt.user_id = u.id
                   WHERE prt.token = :token AND prt.user_type = 'user' 
    ...
    $updateQuery = "UPDATE users SET password_hash = :password WHERE id = :user_id";
    $updateStmt = $db->prepare($updateQuery);
    $updateStmt->bindValue(':password', $hashedPassword);
    $updateStmt->bindValue(':user_id', $tokenData['user_id']);
    $updateStmt->execute();
    ...
    $logQuery = "INSERT INTO activity_logs (user_id, user_type, action, entity_type, entity_id, ip_address) 
                 VALUES (:user_id, 'user', 'password_reset', 'user', :user_id, :ip_address)";
    $logStmt = $db->prepare($logQuery);
    $logStmt->bindValue(':user_id', $tokenData['user_id']);
