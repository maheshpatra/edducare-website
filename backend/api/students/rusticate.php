...
try {
    ...
    // UPDATE EXPLICITLY
    // We set is_active=0 (int) and status='rusticated' (string)
    $updateQuery = "UPDATE students SET status = 'rusticated', is_active = 0, updated_at = NOW() WHERE id = :id";
    ...
    $logQuery = "INSERT INTO activity_logs (user_id, user_type, school_id, action, entity_type, entity_id, ip_address) 
                 VALUES (:user_id, 'user', :school_id, 'rusticate_student', 'student', :entity_id, :ip)";
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Student rusticated successfully'
    ]);
