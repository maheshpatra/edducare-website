...
export default function Admission() {
  ...
  if (isSuccess) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 py-24 text-center px-4">
      ...
      <p className="text-xl text-slate-500 max-w-xl mx-auto mb-6 leading-relaxed">Your admission application for {activeSchool?.name || 'our school'} has been received. We will contact you shortly.</p>
      <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-12">Reference: #ADM-{Math.floor(100000 + Math.random() * 900000)}</p>
      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-xl text-slate-500 max-w-2xl mx-auto leading-relaxed">
            Secure your child's future. Join our vibrant learning community.
          </motion.p>
        </div>
            ...
            {step === 1 && (
              <div className="space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div>
                    <label className="block text-slate-500 text-xs font-bold uppercase tracking-widest mb-3">First Name</label>
                    <input type="text" required className="w-full px-6 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:border-primary focus:bg-white focus:outline-none transition-all" placeholder="e.g. John" />
                  </div>
                  <div>
                    <label className="block text-slate-500 text-xs font-bold uppercase tracking-widest mb-3">Last Name</label>
                    <input type="text" required className="w-full px-6 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:border-primary focus:bg-white focus:outline-none transition-all" placeholder="e.g. Doe" />
                  </div>
                  <div>
                    <label className="block text-slate-500 text-xs font-bold uppercase tracking-widest mb-3">Date of Birth</label>
                    <input type="date" required className="w-full px-6 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:border-primary focus:bg-white focus:outline-none transition-all" />
                  </div>
                  <div>
                    <label className="block text-slate-500 text-xs font-bold uppercase tracking-widest mb-3">Grade Applying For</label>
                    <select className="w-full px-6 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:border-primary focus:bg-white focus:outline-none transition-all">
            ...
            {step === 2 && (
              <div className="space-y-10">
                <div className="flex items-center space-x-5 pb-6 border-b border-slate-100">
                  ...
                  <div>
                    <h3 className="text-2xl font-extrabold text-slate-900">Guardian Details</h3>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Contact information</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="md:col-span-2">
                    <label className="block text-slate-500 text-xs font-bold uppercase tracking-widest mb-3">Guardian Name</label>
                    <input type="text" required className="w-full px-6 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:border-primary focus:bg-white focus:outline-none transition-all" placeholder="Full name" />
                  </div>
                  <div>
                    <label className="block text-slate-500 text-xs font-bold uppercase tracking-widest mb-3">Contact Number</label>
                    <input type="tel" required className="w-full px-6 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:border-primary focus:bg-white focus:outline-none transition-all" placeholder="+91 XXXXXXXXXX" />
                  </div>
                  <div>
                    <label className="block text-slate-500 text-xs font-bold uppercase tracking-widest mb-3">Email</label>
                    <input type="email" required className="w-full px-6 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:border-primary focus:bg-white focus:outline-none transition-all" placeholder="email@example.com" />
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-10">
                <div className="flex items-center space-x-5 pb-6 border-b border-slate-100">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Upload required files</p>
                  </div>
                </div>
                <div className="p-10 border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50/50 hover:border-primary/30 hover:bg-primary/5 transition-all text-center cursor-pointer group">
                  <Upload className="w-14 h-14 text-primary/30 mx-auto mb-6 group-hover:scale-110 group-hover:text-primary transition-all" />
                  <p className="text-lg font-bold text-slate-800 mb-2">Upload Birth Certificate</p>
                  <p className="text-slate-400 text-sm">Click to select or drag and drop (Max 5MB, PDF/JPG)</p>
                </div>
                <div className="flex items-start space-x-4 p-6 bg-blue-50 border border-blue-100 rounded-2xl text-blue-600">
                  ...
                  <p className="text-sm font-medium leading-relaxed">By submitting, you agree to the school's privacy policy and terms of admission.</p>
                </div>
              </div>
            )}
